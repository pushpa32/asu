...
async function otpFunction(emp_id, phone, otp) {
  const otpCheck = await Otp.findOne({ emp_id: emp_id });
  if (otpCheck) {
    await Otp.deleteOne({ emp_id: emp_id })
  }
  ...
  if (await otpSave.save()) return 1;
  else return 2;
}
...
export const loginPhone = async (req, res) => {
  ...
  try {
    ...
    if (userCheck) {
      ...
      if (userCheck.device_id != null) {
        if (userCheck.device_id === req.body.device_id) {
          ...
          if (comparing != null) {
            out.status = "NoDevice"
            out.message = "Device or Phone Number is already in Use!";
            out.error = true;
            out.data = null;
    ...
    } else throw Error("Wrong Phone Number");
  } catch (err) {
